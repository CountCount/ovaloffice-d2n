...
foreach ($oldtown_seasons AS $sno => $data) {
	...
	print '<tr><th></th><th>'.t('STAT_CITY_NAME').'</th><th style="font-size:.75em;text-align:center;">Spy OO</th><th>'.t('POINTS').'</th><th>'.t('STAT_LAST_DAY').'</th><th>'.t('STAT_CHAOS').'</th><th>'.t('STAT_DEVAST').'</th></tr>';
	print $data;
	print '</table>';
}
print '</div>';
...
foreach ($seasons AS $s => $sdates) {
	...
	print $r1 ? '<tr class="odd"><th>'.t('REC_ZOMBIE_COUNT').'</th><td>'.$r1['z'].' ('.$r1['name'].')</td><td>'.($r1['id'] >= 11111 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r1['id'].','.($r1['day']+1).');" />' : '').'</td></tr>' : '';
	print $r2 ? '<tr class="even"><th>'.t('REC_DEFENSE').'</th><td>'.$r2['v'].' ('.$r2['name'].')</td><td>'.($r2['id'] >= 11111 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r2['id'].','.($r2['day']+1).');" />' : '').'</td></tr>' : '';
	print $r3 ? '<tr class="odd"><th>'.t('REC_DEFENSE_10').'</th><td>'.$r3['v'].' ('.$r3['name'].')</td><td>'.($r3['id'] >= 11111 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r3['id'].','.($r3['day']+1).');" />' : '').'</td></tr>' : '';
	print $r4 ? '<tr class="even"><th>'.t('REC_DEFENSE_15').'</th><td>'.$r4['v'].' ('.$r4['name'].')</td><td>'.($r4['id'] >= 11111 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r4['id'].','.($r4['day']+1).');" />' : '').'</td></tr>' : '';
	print '</table>';
