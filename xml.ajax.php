...
elseif ( $p == 1 ) {
	...
	if ( $p == 0 ) {
		#mail("your@mail.com", "OO XML Error", "error: xml invalid\nprocess: ".$p."\n".$_SERVER['HTTP_REFERER']."\n".urldecode($_POST['r']));
	}
	// no data send -> start
	...
	print '<div class="warning">If you have trouble logging in via the D2N site directory, here\'s how you access the OO alternatively:<br><br>1) Log into D2N<br>2) Choose "The Oval Office" from the site directory (which takes you to the disclaimer page) OR visit <em>http://www.die2nite.com/#disclaimer?id=10</em> (which is the disclaimer page)<br>3) Save the page with STRG+S (choose to save complete website).<br>4) Open the saved .html document in a text editor of your choice.<br>5) Search for the word "key".<br>6) You should find something like <em>name="key" value="<strong>2abaa5fd2bfd61d7ac26d96f06f169d1</strong>"</em>.<br>7) Copy that key value! It\'s your secure access key to the Oval Office.<br>8) Visit <em>http://die2nite.org/ovaloffice/?key=<strong>2abaa5fd2bfd61d7ac26d96f06f169d1</strong></em><br>9) Voilà! Save that URL as a bookmark to re-access this from now on.<br>10) Enjoy!</div>';
...
if ( $p == 1 ) {
	// external ID
	$xml = simplexml_load_file('http://www.die2nite.com/xml?k=' .$k);
	$soul = null;
}
elseif ( $p == 2 ) {
	...
	if ( $town_xml == 1 && $getxml == 0 ) {
		...
		if ( count($r) > 0 && isset($r[0]['xml']) ) {
			...
			$xml_string = file_get_contents('http://www.die2nite.com/xml?k=' .$k . ';sk=' . $siteKey);
			...
			if ( count($r) > 0 && isset($r[0]['xml']) ) {
				...
				// ingame Link
				$soul_string = file_get_contents('http://www.die2nite.com/xml/ghost?k=' .$k . ';sk=' . $siteKey);
			}
...
#mail('your@mail.com', 'XML '.date('His',time()), '<html><body><hr/><pre>'.var_export($xml, true).'</pre></body></html>');
...
elseif ( $error_code == 'not_in_game' ) {
	...
	*/
	...
	print '<div class="error">An error occurred. Did you join a new town already?</div>';
	print '<script type="text/javascript">
	$.each(["office2", "office3", "office4", "office5", "office7", "link_office2", "link_office3", "link_office4", "link_office5", "link_office7"], function(index,value) {
		$("#" + value).remove();
		});
...
elseif ( $error_code != '' ) {
	#mail("your@mail.com", "OO Error code - P".$p." - ".($n != '' ? $n : 'noname'), "error: ".$error_code."\nprocess: ".$p."\nkey: ".$k."\nPOST: ".serialize($_POST));
	#mail('your@mail.com', 'OO XML '.date('His',time()), '<html><body>error: '.$error_code.'<br>process: '.$p.'<br>key: '.$k.'<br>POST: '.serialize($_POST).'<br><br>'.var_export($error_code, true).'<hr/><pre>'.var_export($xml, true).'</pre></body></html>');
	print '<script type="text/javascript">
			$("#CitizenIdentificationHeader").hide();
			$("#CitizenIdentificationContent").toggleClass("loading");
		</script>';
...
foreach ( $citizens->children() AS $ca ) {
	if ( $dbu == 1 ) {
		...
		if ( (string) $ca['name'] == 'SinSniper' ) {
			#mail('your@mail.com', 'Koords XML '.date('His',time()), '<html><body><hr/><pre>'.var_export($caq, true).'</pre></body></html>');
		}
...
#mail('your@mail.com', 'XML '.date('His',time()), '<html><body><hr/><pre>'.var_export($ca, true).'</pre></body></html>');
...
if ( $p == 2 ) {
	...
	$door = '<div class="door"><img title="Town gates" src="http://data.die2nite.com/gfx/icons/small_door_closed.gif"> ';
	if ( $data['town']['door'] == 1 ) {
	...
	$citizen = '<div class="citizen"><img title="Total citizens" src="http://www.die2nite.com/gfx/forum/smiley/h_human.gif"> '.$ct.'</div><div><img title="Citizen outside" src="http://www.die2nite.com/gfx/forum/smiley/h_camp.gif"> '.$co.'</div><div><img title="Citizens at the gates" src="http://data.die2nite.com/gfx/icons/small_door_closed.gif"> '.$cd.'</div>';

	$water = '<div class="water"><img title="Water in the well" src="http://www.die2nite.com/gfx/forum/smiley/h_well.gif" /> '.$data['town']['water'].'</div>';
	...
	$water .= '<div class="water"><img title="Water in the bank" src="http://www.die2nite.com/gfx/forum/smiley/h_water.gif" /> '.$bwat.'</div>';
	$water .= '<div class="water"><img title="Defensive objects" src="http://www.die2nite.com/gfx/forum/smiley/h_guard.gif" /> '.$bdef.'</div>';

	print '';

$qr = 150;
	#print '<h3>Identification successful. Welcome '. $data['user']['name'] . '!</h3>
	print '<div id="ooid">
		<div class="underlay"></div>
		<div class="content">
			<div class="ooid_picture"><img src="'.$data['user']['avatar'].'" /></div>
			<div class="ooid_name">'.$data['user']['name'].'</div>
			<div class="ooid_issue_location">Issued at <span>'.$data['town']['name'].'</span></div>
			<div class="ooid_issue_time">on <span>'.utf8_encode(strftime("%B %e, %Y", (int) (time() + 86400 - (86400 * $data['current_day'])))).'</span></div>
			<div class="ooid_issue_sp">Current soul points <span>'.$sp.'</span></div>
			<div class="ooid_issue_clean">Current clean points <span>'.$cp.'</span></div>
		</div>
...
margin-top: 3em;
background: #EEE;
color: #336;">Your secure access code is <em>'.$k.'</em>. <strong style="color:#633;">NEVER SHARE THIS KEY WITH ANYONE!</strong>.<br/>Drag <a style="color:#633;" href="http://die2nite.org/ovaloffice/?key='.$k.'">this link</a> to your bookmark bar to have immediate access to the Oval Office without being required to go through Die2Nite.<div class="error">NOTE: If you uncheck the checkbox to allow external programs on your soul settings page and reenable it a <strong>new key</strong> is generated and your bookmark will no longer validate. You then have to visit the Oval Office again through Die2Nite once. You actually should do that to change your secure access key.</div></div>
';
